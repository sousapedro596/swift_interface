...
//Marker with orientation

function gmap_deleteRotationalMarker(key){

	rotational_markers[key].setMap(null);
	delete rotational_markers[key];
...
function gmap_addRotationalMarker(key, latitude, longitude, parameters)
{

	if (key in rotational_markers) {
		gmap_deleteRotationalMarker(key);
	}
	...
	var icon =  {
        ...
        rotation: parameters['rotation']-90,
	...
	rotational_markers[key] = marker;
	return key;
}



function gmap_moveRotationalMarker(key, latitude, longitude, rotation){




	var icon = rotational_markers[key].getIcon();

	icon['rotation'] = rotation -90;

	rotational_markers[key].setIcon(icon);

	var coords = new google.maps.LatLng(latitude, longitude);
	rotational_markers[key].setPosition(coords);
